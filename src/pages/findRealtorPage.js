...
export default function FindRealtor() {
    ...
    useEffect(() => {
      async function getRealtors() {
        ...
        setState({ ...state, realtors: result, realtorSelected: result[0], languages: language });
        console.log(result);
      }
      getRealtors();
    }, []);
    ...
    const searchRealtors = async (e) => {
      const results = await axios.get(`https://localhost:44340/api/directory/realtors/name/${e.target.value}`)
      const result = results.data
      if(result.length > 0) {
        setState({...state, realtors: result, realtorSelected: result[0], showReviews: false})
      }
    }

    const filterByLanguage = async (e) => {
      const results = await axios.get(`https://localhost:44340/api/directory/realtors/lang/${e.target.value}`)
      const result = results.data
      if(result.length > 0 ) {
        setState({...state, realtors: result, realtorSelected: result[0], showReviews: false})
      }
    ...
    const companies = [
      ...new Set(state.realtors.map((realtor) => realtor.companyName)),
    ];
            ...
            <section className="flex flex-col flex-auto border-gray-800 overflow-y-scroll">
              ...
              <article>
                ...
                <div className="mt-6 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                  {state.showReviews ? (
                    <>
                      <div className="max-h-64 overflow-hidden overflow-y-auto">
                          ...
                          <div className="min-w-0 flex-1">
                            <form onSubmit={postReview}>
                              ...
                              <p className="text-sm leading-5 text-gray-500 mx-4 mt-4">
                                How would you rate your expiereince with this
                                realtor?
                              </p>
