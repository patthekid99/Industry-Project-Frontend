...
export default function FindRealtor() {
    const [state, setState] = useState({showContacts: true, realtors: [], realtorSelected: {}, showReviews: false, reviews: [], comment: "", starRating: 5})

    useEffect(() => {
        async function getRealtors() {
            const results = await axios.get('https://localhost:44340/api/directory/realtors')
    ...
    const getReviews = async () => {
        ...
        const results = await axios.get(`https://localhost:44340/api/review/realtor/${state.realtorSelected.realtorId}`, {
            headers: {
        ...
        const result = results.data
        console.log(result)
        setState({...state, showReviews: true, reviews: result, comment: "", starRating: 5})
    }

    const postReview = async () => {
        var mydata = JSON.parse(localStorage.getItem("myData"))
        const data = {comment: state.comment, starRating: state.starRating}
        const results = await axios.post(`https://localhost:44340/api/review/realtor/${state.realtorSelected.realtorId}`, data ,{
            headers: {
                Authorization: `Bearer ${mydata.tokenString}`,
              },
        })
  ...
  <div className="flex-1 flex flex-col max-w-7xl mx-auto sm:rounded-[24px] bg-white">
    <main className="flex-grow flex flex-row min-h-0">
        ...
        <div className="contacts p-2 flex-1 overflow-y-scroll">
          {state.realtors.map((r) => (
            <div
              ...
              onClick={() => setState({...state, realtorSelected: r, showContacts: false, showReviews: false})}
            >
      ...
      <nav className="flex items-start px-4 py-3 sm:px-6 lg:px-8 md:hidden" aria-label="Breadcrumb">
                  <button href="#" className="inline-flex items-center space-x-3 text-sm font-medium text-gray-900" onClick={() => setState({...state, showContacts: true})}>
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="hidden sm:block 2xl:hidden mt-6 min-w-0 flex-1">
                <h1 className="text-2xl font-bold text-gray-900 truncate">
                  {state.realtorSelected.firstName + " " + state.realtorSelected.lastName}
                </h1>
              </div>
            </div>
          </div>
                          ...
                          <div>
                            <div className="text-sm">
                              <a href="#" className="font-medium text-gray-900">
                                {review.potentialBuyer.firstName + " " + review.potentialBuyer.lastName}
                              </a>
                            </div>
